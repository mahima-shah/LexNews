console.log("SCRIPT STARTED - Verbatim News Network");
...
const FEED_URL = "https://verbatimnewsnetwork.com/feed/";
...
for (const item of selectedItems) {
  ...
  articles.push({
    title: item.title,
    subtitle: item.description?.replace(/<[^>]*>/g, "").slice(0, 180) || "Latest legal news from Verbatim News Network.",
    body: articleText || item.description?.replace(/<[^>]*>/g, "") || item.title,
    category: categorizeArticle(item.title, articleText),
    source_name: "Verbatim News Network",
