console.log("SCRIPT STARTED - Economic Times Legal");
...
const FEED_URL = "https://legal.economictimes.indiatimes.com/rss/recentstories";
...
for (const item of selectedItems) {
  ...
  articles.push({
    title: item.title,
    subtitle: item.description?.replace(/<[^>]*>/g, "").slice(0, 180) || "Latest legal news from Economic Times.",
    body: articleText || item.description?.replace(/<[^>]*>/g, "") || item.title,
    category: categorizeArticle(item.title, articleText),
    source_name: "Economic Times Legal",
